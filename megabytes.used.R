...
megabytes.used.list <- list()
for(pfun.name in names(pfun.list)){
  pfun <- pfun.list[[pfun.name]]
  mem.result <- test.funs(pfun)
  megabytes.used.list[[pfun.name]] <-
    data.frame(pfun.name, mem.result)
}
megabytes.used <- do.call(rbind, megabytes.used.list)

save(megabytes.used, file="megabytes.used.RData")
